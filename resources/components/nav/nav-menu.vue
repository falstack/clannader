...
<template>
    <div class="warp">
        <div class="head" v-if="$store.getters.isLogin">
            <router-link :to="'/people/' + $parent.$data.user.zone" class="uface"><img :src="$getUserInfo('avatar')"></router-link>
            <div class="info">
                <router-link :to="'/people/' + $parent.$data.user.zone">{{ $parent.$data.user.name }}</router-link>
            </div>
        </div>
